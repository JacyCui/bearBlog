---
title: 【静态分析-第9讲】静态分析与安全
abbrlink: 20983
keywards: static-analysis
tag: 静态分析
mathjax: true
date: 2022-09-09 19:27:24
description: 介绍信息流安全的相关概念，以污点分析为例讲解静态分析在信息流安全中的应用。
cover: https://s2.loli.net/2022/09/09/l1OYDMpKC8trUTX.png
---
...
{% label 定义9.1 blue %}：称一个系统是**安全的（secure）**，如果它能够在存在**对手（adversaries）**的情况下达成原本的目标。
{% endnote %}
...
{% note blue 'fas fa-book' flat %}
{% label 定义9.1 blue %}：**访问控制（Access Control）** 是指对于特定的信息检查请求访问该信息的程序是否具有相应的权限（rights）或者是否得到了相应的许可（permissions）。
{% endnote %}
...
{% note blue 'fas fa-book' flat %}
{% label 定义9.2 blue %}：**信息流安全（Information Flow Security）**是指通过追踪信息是如何在程序中流动的方式，来确保程序安全地处理了它所获得的信息。
{% endnote %}
...
{% note blue 'fas fa-book' flat %}
{% label 定义9.3 blue %}：如果变量 $x$ 中的信息转移到了变量 $y$ 中，则存在**信息流（Information Flow）** $x \to y$ 。
{% endnote %}
...
<img src="https://s2.loli.net/2022/09/10/5PO4imoGkxpVwR9.png" alt="security-levels" style="zoom:40%;" />

## 信息流政策

{% note blue 'fas fa-book' flat %}
{% label 定义9.4 blue %}：**信息流政策（Information Flow Policy）**是指对于不同**安全等级（Security Level）**之间信息流动的限制。
{% endnote %}

{% note blue 'fas fa-book' flat %}
{% label 定义9.5 blue %}：**不介入政策（Noninterference Policy）**是一种信息流政策，它要求高安全等级的变量的信息不应当作用于，或者说不应当干涉低安全等级变量的信息。
{% endnote %}

<img src="https://s2.loli.net/2022/09/10/aEJjI4HzMwcFgVZ.png" alt="low-high" style="zoom:40%;" />

直观地理解就是，你不应当能够通过观察低安全等级变量的方式得出任何关于高安全等级变量的结论。

<img src="https://s2.loli.net/2022/09/10/bZY4NJBwpjzvTxl.png" alt="noninterference" style="zoom:50%;" />
...
{% note blue 'fas fa-book' flat %}
{% label 定义9.6 blue %}：**机密性（Confidentiality）**指的是阻止机密的信息泄漏，**完整性（Integrity）**指的是阻止不信任的信息污染受信任的关键信息。
{% endnote %}

这是另一种信息流分析的视角（每个视角下有自己的安全性分类策略）：
- 在机密性层面，信息流只能从低机密性流向高机密性；
    - 考虑机密性的时候信息分类为**保密（Secret）**和**公开（Public）**。
    <img src="https://s2.loli.net/2022/09/10/WmeQJ6ApxvSnEwT.png" alt="confidentiality" style="zoom:50%;" />
    - 机密性可以理解成是一种“读保护”。
- 在完整性方面，信息流只能从高完整性流向低完整性。
    - 考虑完整性的时候信息分类为**信任（Trusted）**和**不信任（Untrusted）**。
    <img src="https://s2.loli.net/2022/09/10/LWFmovRwKr7gEAP.png" alt="integrity" style="zoom:50%;" />
    - 完整性可以理解成是一种“写保护”。
...
{% note blue 'fas fa-book' flat %}
{% label 定义9.7 blue %}：**广义的完整性（Broad Definition of Integrity）**是指保证数据的正确性（Correctness），完全性（Completeness）以及一致性（Consistency）。
{% endnote %}
...
{% note blue 'fas fa-book' flat %}
{% label 定义9.8 blue %}：信息可以通过直接拷贝的方式进行流动，这样的信息流称为**显式流（Explicit Flow）**。
{% endnote %}
...
{% note blue 'fas fa-book' flat %}
{% label 定义9.9 blue %}：信息可以通过影响控制流的方式向外传递，这样的信息流称为**隐式流（Implicit Flow）**。
{% endnote %}

## 隐藏通道

{% note blue 'fas fa-book' flat %}
{% label 定义9.10 blue %}：称在计算系统中指示信息的机制为**信道（Channels）**。如果这个指示信息的机制的本意并不是信息传递，这样的信道称为**隐藏信道（Covert/Hidden Channels）**。
{% endnote %}
...
{% note blue 'fas fa-book' flat %}
{% label 定义9.11 blue %}：**污点分析（Taint Analysis）**将程序中的数据分为两类：
...
<img src="https://s2.loli.net/2022/09/10/Ttvq5AXUxZbPowF.png" alt="taint-analysis-eg" style="zoom:30%;" />
